...
def signal_note(metric: str) -> str:
    ...
    if metric == "assessment_objective_similarity":
        return " (mean-max rectified cosine max(0,cos) on [0,1]; NON-DIRECTIONAL)"
...
def gen_table(pooled: dict, blocks: list[dict]) -> str:
    ...
    head = [
        header_comment(pooled, "RQ2 instrument sensitivity, one block per swept "
                               "axis: level on that axis's signal, rank agreement "
...
def gen_macros(pooled: dict, blocks: list[dict]) -> str:
    ...
    tex = [header_comment(
        pooled,
        "RQ2 instrument-sensitivity macros (encoder axis + Bloom-classifier "
        "axis); each axis is reported on the signal it can move, and signals are "
...
def gen_stats_md(pooled: dict, blocks: list[dict]) -> str:
    ...
    out = [
        "<!-- AUTO-GENERATED by isd-agent-benchmark/scripts/alignmentgraph-isd-bench/"
        "14_gen_sensitivity_table.py — DO NOT EDIT BY HAND -->",
        "# RQ2 instrument sensitivity (star of single-axis arms)",
        "",
        f"- source pooled JSON generated_at: `{pooled.get('generated_at', '?')}`",
        ...
        f"- each axis is reported on a signal that axis can move: {axis_line}. "
        ...
        "Bloom level.",
        "- RQ2 is a flat 7-signal panel with NO primary endpoint; the two signals "
        "below are each axis's sensitivity lead, not an endpoint.",
        "- similarity signals are mean-max rectified cosine, `max(0, cos)` on "
        "[0,1]; the protocol has no similarity threshold anywhere.",
