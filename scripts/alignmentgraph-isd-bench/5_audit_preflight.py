...
Checks, per ladder slot found in .env (managed block or manual quads):
  1. BASE_URL is set and the pod answers /v1/models: 401 without key,
  ...
  2. The three protective flags are set to the required values
     (AGENT_MODEL_MAX_TOKENS_CAP=8096, AGENT_MODEL_STREAMING=1,
    ...
    for slot in SLOTS:
        ...
        if status == 200:
            ...
            if not args.no_smoke:
                ...
                report(st == 200 and content_ok, "smoke: content non-empty",
                       f"status={st}")
                report(no_think, "smoke: no reasoning_content (non-thinking)",
