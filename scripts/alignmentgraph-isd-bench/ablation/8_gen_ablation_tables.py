...
Companion of ../8_gen_paper_tables.py — same conventions (booktabs, Vietnamese
decimal comma written as ``{,}``, AUTO-GENERATED headers, no hand-typed
numbers), fed by ablation/7_pool_ablation_runs.py instead of the ladder pool.
...
Usage:
  python scripts/alignmentgraph-isd-bench/ablation/8_gen_ablation_tables.py
  python scripts/alignmentgraph-isd-bench/ablation/8_gen_ablation_tables.py \
...
# ── formatting (same conventions as ../8_gen_paper_tables.py) ────────────────
...
def header_comment(pooled: dict, what: str) -> str:
    return "\n".join([
        ...
        "ablation/8_gen_ablation_tables.py — DO NOT EDIT BY HAND",
        f"% {what}",
...
def gen_tab_ablation(pooled: dict) -> str:
    ...
    lines.append(
        ...
        " \\texttt{test\\_90} theo thang kích thước mô hình (1 run mỗi kích"
        " thước; cả 4 arm chấm trong cùng judge session). Total = trung bình"
...
def gen_stats_md(pooled: dict) -> str:
    ...
    L.append("")
    L.append(f"AUTO-GENERATED by ablation/8_gen_ablation_tables.py at "
    ...
    L.append("## RQ2 alignment composite (arm − A0, paired)")
    L.append("")
    if align and align.get("stats", {}).get("per_model"):
        ...
        L.append("")
        L.append("| Size | Arm | Δ composite | CI95 | p_holm | n |")
        L.append("|---|---|---|---|---|---|")
        ...
        L.append("Absent — run scripts/alignmentgraph-isd-bench/7_score_alignment.py "
                 "on each ablation run dir, then re-run "
                 "ablation/7_pool_ablation_runs.py and this script.")
    L.append("")
    return "\n".join(L) + "\n"
